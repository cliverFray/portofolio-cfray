...
import { FaJava, FaPython, FaReact, FaDocker, FaAws, FaGitAlt,FaLinkedin, FaGithub, FaInstagram, FaPhone } from "react-icons/fa";
import { SiSpringboot, SiCplusplus, SiFlutter, SiPostgresql, SiTensorflow } from "react-icons/si";
import "./index.css";
  ...
  const run = () => {
    interval = setInterval(() => {
      if (!deleting) {
        ...
        i--;
        setText(fullText.slice(0, i));
        if (i === 1) {           // 👈 nunca borra hasta 0
          deleting = false;
          clearInterval(interval);
          setTimeout(run, 500);  // pausa antes de volver a escribir
        }
      }
    }, deleting ? 60 : 80);      // velocidad distinta para escribir/borrar
  };
  ...
  return (
    <div className="min-h-screen relative bg-[#0f1724] text-[#e6eef8] font-sans overflow-hidden">
      ...
      <main className="max-w-[1100px] mx-auto px-6 py-12 relative z-10">
        {/* HEADER */}
        <motion.header {...fadeIn("down", 0.1)} className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div
              ...
              style={{
                ...
                transform: `translateY(${Math.sin(t) * 4}px) rotate(${Math.sin(t / 3) * 2}deg)`,
              }}
            >
              CF
            </div>
            <div>
              <h1 className="text-lg md:text-xl font-semibold">Cliver Fray Aguilar Sulca</h1>
          ...
          <nav className="hidden sm:flex gap-4 text-sm text-gray-400">
            <a href="#projects" className="hover:text-[#00f5ff]">Proyectos</a>
            <a href="#experience" className="hover:text-[#00f5ff]">Experiencia</a>
            <a
              href="#contact"
  ...
  <div className="mt-4">
    ...
    <div className="flex flex-wrap gap-2 text-xs text-gray-400">
      {[
        ...
        {/* MÚSICA */}
        <motion.section id="music" {...fadeIn("up", 0.2)} className="mt-16 text-center relative">
